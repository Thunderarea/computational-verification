...
import gr.iti.mklab.extractfeatures.TweetFeaturesExtractor;
...
public class WebOfTrustManager {
	...
	private static WebOfTrustManager mInstance = new WebOfTrustManager();
	
	private static final Logger LOGGER = Logger.getLogger( TweetFeaturesExtractor.class.getName() );

	public static WebOfTrustManager getInstance() {
		
		
		if (mInstance == null) {			
			mInstance = new WebOfTrustManager();
		}
		return mInstance;
	 ...
	 */
	public String handleInstagram(String url) {

		int indexInstagram = url.indexOf("instagr");
		LOGGER.info("index insta " + indexInstagram);
		String charInstagram = String.valueOf(url.charAt(indexInstagram + 7));
		LOGGER.info(" charInstagram " + charInstagram);
		if (charInstagram.equals(".")) {
			url = url.replaceAll("instagr.am", "instagram.com");
			LOGGER.info("instagram " + url);
		}
		LOGGER.info("handle instagram :" + url);
		return url;
	}
