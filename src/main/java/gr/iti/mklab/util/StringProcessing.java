...
public class StringProcessing {
	...
	public boolean isAppropriateWord(String str) {
		...
		ClassLoader classLoader = StringProcessing.class.getClassLoader();
		...
		try {
			stop_eng	= FileManager.getInstance().loadSetfromFile(classLoader.getResource("/stop_words/stop-words-eng.txt").getFile());
			stop_es 	= FileManager.getInstance().loadSetfromFile(classLoader.getResource("/stop_words/stop-words-es.txt").getFile());
			stop_tweets = FileManager.getInstance().loadSetfromFile(classLoader.getResource("/stop_words/stop-words-twitter.txt").getFile());
		} catch (IOException e) {
			e.printStackTrace();
		}
